...
//				.paths("/")
	...
	/**
	 * 
	 * @return ApiInfo object ApiInfo allows to provide Customized information for
	 *         Docket Swagger_2 Customized Information - Title for API Description
	 *         of API Terms of Service Url License Contact Information Version of
	 *         API
	 * 
	 */
	private ApiInfo apiDetails() {
		ApiInfo apiInfo = new ApiInfoBuilder().title(env.getProperty("string.swagg.title"))
				.description(env.getProperty("string.swagg.desc"))
				.termsOfServiceUrl(env.getProperty("string.swagg.help")).license(env.getProperty("string.swagg.lic"))
				.contact(new Contact(env.getProperty("conctact.name"), env.getProperty("contact.web"),
						env.getProperty("contact.email")))
				.version(env.getProperty("string.swagg.ver")).build();
